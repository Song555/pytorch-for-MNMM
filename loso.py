...
from loso_models import PixelTransformerResnet, Resnet50
# from models import Resnet50
import copy
import time
import numpy as np
...
for ai in range(1, 69):
    ...
    train_path = os.path.join(r'data\loso_image_LVMM3\train', train_name)
    #train_path = os.path.join(r'data\Microexpressions\out2\apex', train_name)
    ...
    test_path = os.path.join(r'data\loso_image_LVMM3\test', test_name)
    #test_path = os.path.join(r'data\Microexpressions\out2\apex', test_name)
    ...
    train_label_path = r'data\loso_image_LVMM3\train_label' + '/' + train_label_name
    ...
    test_label_path = r'data\loso_image_LVMM3\test_label' + '/' + test_label_name
    ...
    #net = PixelTransformerResnet(mid_dim=256, last_dim=n_classes, dropout=dropout, n_heads=n_heads)
    '''
    print(net)
    ...
    def test():
        #path=os.path.join("./data/out2/pth",("newbest"+str(ai)+".pth"))
        # print(11111)
        #print(path)
    ...
    if __name__ == "__main__":
        ...
        for epoch in range(1, epochs + 1):
            global fp
            fp = open('data/out_LVMM3/txt/result' + str(ai) + '.txt', 'a+')
            train(epoch)
            best_model_wts, best_acc, best_pre, best_tar, FP_Epoch, FN_Epoch, TP_Epoch, TN_Epoch, testloss = test()
            # best_model_wts, best_acc, best_pre, best_tar, testloss =  test()
        ...
        torch.save(net.state_dict(), 'data/out_LVMM3/pth/newbest' + str(ai) + '.pth')
        ...
        fp.close()
        fl = open('data/out_LVMM3/label/result_label' + str(ai) + '.txt', 'a+')
        fl.write('Best  pred: {}\n'.format(best_pre))
...
jieguo = open('data/out_LVMM3/uf1_uar.txt', 'a+')
