...
class PixelTransformerResnet(nn.Module):  # (pixel transformer in resnet)
    def __init__(self, mid_dim=256, last_dim=3, dropout=0.1, n_heads=8):
        ...
        #print(list(resnet.children())[-4:-1])
        ct = 0
        ...
        #for i, e in enumerate(resnet.children()):
            #print("第{}个元素为：\n {}".format(i, e))
        #print(list(resnet.children())[-2:-1])
        ...
        self.conv6 = nn.Sequential(
            nn.Conv2d(1024, 1024, kernel_size=1, stride=2, padding=0),
            nn.BatchNorm2d(1024),
            nn.ReLU()
            #, nn.LogSoftmax(dim = 1)
          )
    ...
    def forward(self, x):
        ...
        #x = self.feature_extractor_after_pixelTransformer(x)
        #print(3333)
        #print(x.shape)
